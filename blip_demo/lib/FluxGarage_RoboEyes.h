...
// Use enum instead of macros to avoid conflicts
#define ROBO_N 1
#define ROBO_NE 2
#define ROBO_E 3
#define ROBO_SE 4
...
// Animation - pulsing heart eyes
bool heartEyes = 0;
unsigned long heartEyesStartTime = 0;
int heartEyesDuration = 5000;
float heartPulsePhase = 0;
...
void drawHeartEye(int x, int y, int w, int h) {
  int cx = x + w/2;
  int cy = y + h/2;
  
  float pulseScale = 1.0 + 0.15 * sin(heartPulsePhase);
  int baseSize = min(w, h) / 2;
  int size = baseSize * pulseScale;
  
  // Make sure heart fits within eye bounds
  int heartSize = constrain(size, 2, min(w, h) / 2);
  
  // Left lobe
  display->fillCircle(cx - heartSize/2, cy - heartSize/4, heartSize * 0.7, MAINCOLOR);
  // Right lobe
  display->fillCircle(cx + heartSize/2, cy - heartSize/4, heartSize * 0.7, MAINCOLOR);
  // Bottom point
  display->fillTriangle(cx - heartSize, cy - heartSize/4, cx + heartSize, cy - heartSize/4, cx, cy + heartSize, MAINCOLOR);
  
  // Clean up edges
  display->drawRoundRect(x, y, w, h, eyeLborderRadiusCurrent, BGCOLOR);
}
...
void drawEyes(){
  ...
  // Check if heart eyes animation is active
  if(heartEyes) {
    unsigned long currentTime = millis();
    heartPulsePhase += 0.2;
    
    // Draw heart eyes
    drawHeartEye(eyeLx, eyeLy, eyeLwidthCurrent, eyeLheightCurrent);
    if (!cyclops) {
      drawHeartEye(eyeRx, eyeRy, eyeRwidthCurrent, eyeRheightCurrent);
    }
    
    if(currentTime >= heartEyesStartTime + heartEyesDuration) {
      heartEyes = 0;
    }
  } else {
    // Normal eye drawing
